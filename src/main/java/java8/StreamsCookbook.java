...
public class StreamsCookbook {
	public static final List<StreamsCookbook> tdfWinners = Arrays.asList(
	...
	public static void main(String args[]) {
		...
		List<String> winnersOfToursLessThan3500km = tdfWinners
				...
				.map(StreamsCookbook::getName) // Get names of winners
		...
		// limit -
		List<StreamsCookbook> winnerObjectsOfToursLessThan3500kmLimit2 = tdfWinners
				.stream()
				.filter(d -> d.getLengthKm() < 3500)
				.limit(2)
				.collect(toList());
		...
		// skip records
		List<StreamsCookbook> skipEveryOtherTDFWinner = tdfWinners
				.stream()
				.skip(2)
				.collect(toList());
		...
		Optional<StreamsCookbook> winner2012 = tdfWinners.stream().filter(w -> w.getName().contains("Wiggins")).findAny();
		// winner2012 - Bradley Wiggins
		System.out.println("winner2012 - " + winner2012.get());
		Optional<Integer> winnerYear2014 = tdfWinners.stream().map(StreamsCookbook::getYear).filter(x -> x == 2014).findFirst();
		// winnerYear2014 - 2014
		System.out.println("winnerYear2014 - " + winnerYear2014.get());
		// reducing - 0 --> initial value
		int totalDistance = tdfWinners.stream().map(StreamsCookbook::getLengthKm).reduce(0, Integer::sum);
		// totalDistance - 38767
		System.out.println("totalDistance - " + totalDistance);
		Optional<Integer> shortestYear = tdfWinners.stream().map(StreamsCookbook::getLengthKm).reduce(Integer::min);
		// shortestYear - 3360
		System.out.println("shortestYear - " + shortestYear.get());
		Optional<Integer> longestYear = tdfWinners.stream().map(StreamsCookbook::getLengthKm).reduce(Integer::max);
		// longestYear - 3661
		System.out.println("longestYear - " + longestYear.get());
		Optional<StreamsCookbook> fastestWinner = tdfWinners.stream().min(Comparator.comparingDouble(StreamsCookbook::getAveSpeed));
		System.out.println("fastestTDF - " + fastestWinner.get());
		// shorthand
		OptionalDouble fastestTDF = tdfWinners.stream().mapToDouble(StreamsCookbook::getAveSpeed).min();
		// fastestTDF - 39.0
		System.out.println("fastestTDF - " + fastestTDF.getAsDouble());
		// groupingby - make a map whose keys are names
		Map<String, List<StreamsCookbook>> namesVsWinner = tdfWinners.stream().collect(groupingBy(StreamsCookbook::getName));
		...
		// join strings
		String allTDFWinnersTeamsCSV = tdfWinners.stream().map(StreamsCookbook::getTeam).collect(joining(", "));
		...
		// grouping
		Map<String, List<StreamsCookbook>> winnersByNationality = tdfWinners.stream().collect(groupingBy(StreamsCookbook::getNationality));
		...
		Map<String, Long> winsByNationalityCounting = tdfWinners.stream().collect(groupingBy(StreamsCookbook::getNationality, counting()));
